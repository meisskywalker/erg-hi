...
<template>
  <div class="flex flex-col items-center justify-center container mx-auto min-h-screen gap-4">
    <div>
    ...
    <div class="bg-grey-50 p-8 rounded-md">
      <form class="flex flex-col items-center gap-4" @submit.prevent="login">
        ...
        <div class="flex flex-col gap-2">
          <input-with-label label="Email" class-name="w-72" :value="credentials.email.value" @update="update" />
          <input-with-label label="Password" type="password" class-name="w-72" :value="credentials.password.value"
            @update="update" />
        </div>
        <button type="submit"
